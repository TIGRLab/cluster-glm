...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
sd = which('spm')
VG = [sd(1:end-5) '/templates/EPI.nii,1'];
...
for idx = 1:size(directories,1);
    ...
    op=[''];
    for jdx = 1:size(p,1)
        np(jdx,1:size(p,2)+size(directories,2))= [directories(idx,:)  p(jdx,:)];
        sp(jdx,1:size(p,2)+size(directories,2)+1)= [directories(idx,:) 'w' p(jdx,:)];
        op(jdx,1:size(p,2)+size(directories,2)+2)= [directories(idx,:) 'sw' p(jdx,:)];
    end
    nP=[nP; np];
    oP=[oP; op];
    sP=[sP; sp];
